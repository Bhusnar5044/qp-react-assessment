...
function BoardItemProvider({ children, locationDependenies: { column, taskId } }: BoardItemProps) {
...
function useBoardItem() {
  const context = useContext(BoardItemContext);
  if (!context) throw new Error("Board item context must be used within its provider's scope!");
  return context;
}

export { BoardItemProvider, useBoardItem };
