import { BoardData, BoardTask, ChildrenProp, ErrorMessage } from '@/types/generalTypes';
...
export type formTypeProp = 'create/task' | 'edit/task' | 'create/board' | 'edit/board' | 'edit/board/new-column' | 'none';
  ...
  switch (action.type) {
    ...
    case 'form/edit/task': {
      ...
      return {
        ...state,
        formType: 'edit/task',
        formData: state.boardData?.columns[column].tasks.find((task) => task.id === taskId),
    ...
    case 'board/task/delete': {
      ...
      const updatedBoard = get_boardList_with_deleted_task(locationDependencies);
...
function BoardProvider({ children }: ChildrenProp) {
  const [{ boardData, status, boardDataAll, boardPage, formType, formData, formInitialColumn, showBoardDeleteConfirmation }, dispatch] = useReducer(
    reducer,
    initialState
  );
  ...
  useEffect(() => {
    const getBoards = async (): Promise<BoardData | void> => {
      try {
        ...
        const errorMessage: ErrorMessage = error instanceof Error ? error.message : 'An Error Occured';
...
function useBoard() {
  ...
  if (!context) throw new Error("App context must be use inside its parent's scope!");
  return context;
}
