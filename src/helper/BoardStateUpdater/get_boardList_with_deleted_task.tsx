...
export function get_boardList_with_deleted_task({ page, taskId, column, boardDataAll }: locationDependenciesParams) {
  // Find the index to the task that wants to be moved.
  const targetTaskIndex: number = boardDataAll[page].columns[column].tasks.findIndex((task) => task.id === taskId);
