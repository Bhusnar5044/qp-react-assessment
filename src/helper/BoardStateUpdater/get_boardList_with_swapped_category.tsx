...
export function get_boardList_with_swapped_category({
  ...
  const newColumnIndex: number = boardDataAll[page].columns.findIndex((column) => {
    return column.name === newColumnName;
  });

  // Find the index to the task that wants to be moved.
  const targetTaskIndex: number = boardDataAll[page].columns[oldColumnIndex].tasks.findIndex((task) => task.id === taskId);

  // Retrieve the target task object.
  const targetTask: BoardTask = boardDataAll[page].columns[oldColumnIndex].tasks[targetTaskIndex];
  ...
  const removedTargetTaskList: BoardTask[] = removeElementFromArray(updatedBoard[page].columns[oldColumnIndex].tasks, targetTaskIndex);

  // Create a new array of the new column with the added  target task to the array
  const addedTargetTaskList: BoardTask[] = [...updatedBoard[page].columns[newColumnIndex].tasks, { ...targetTask, status: newColumnName }];
