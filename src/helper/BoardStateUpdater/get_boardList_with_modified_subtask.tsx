...
export const get_boardList_with_modified_subtask = ({
  ...
  // find the taskIndex
  const taskIndex: number = boardDataAll[page].columns[column].tasks.findIndex((task) => task.id === taskId);

  if (taskIndex === -1) throw new Error('Update subtask : task Index not found');

  // find the subtaskIndex
  const subtaskIndex: number = boardDataAll[page].columns[column].tasks[taskIndex].subtasks.findIndex((subtask) => subtask.id === subtaskId);

  if (subtaskIndex === -1) throw new Error('Update subtask : subtask Index not found');
  ...
  (updatedBoard[page].columns[column].tasks[taskIndex].subtasks[subtaskIndex] as Record<string, string | boolean>)[key] = value;
