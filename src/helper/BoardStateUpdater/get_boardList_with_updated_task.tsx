import { BoardColumnType, BoardData, BoardTask } from '@/types/generalTypes';
...
export function get_boardList_with_updated_task({
  ...
  const newColumn: number = updatedBoard[page].columns.findIndex((column: BoardColumnType) => column.name === newColumnName);
  ...
  if (newColumn !== initialColumn) {
    ...
    updatedBoard[page].columns[initialColumn].tasks = updatedBoard[page].columns[initialColumn].tasks.filter(
    ...
    const newTaskList: BoardTask[] = updatedBoard[page].columns[newColumn].tasks.reduce((newList: BoardTask[], current: BoardTask) => {
