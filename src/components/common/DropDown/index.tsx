...
export const DropDown: FC<DropDownProps> = ({ onSelect, optionList, className = '', value }) => {
  const [selected, setSelected] = useState(() => value);
  const [isOpen, setIsOpen] = useState(() => false);
  const [direction, setDirection] = useState<'up' | 'down'>(() => 'down');

  // Options * amount + padding Y * 2
  const [optionsHeight] = useState(() => OPTIONS_ELEMENT_HEIGHT_PX * optionList.length + OPTIONS_ELEMENT_PADDING_Y_PX * 2);
  ...
  useLayoutEffect(() => {
    ...
    const ElementDistanceFromBottom = window.innerHeight - selectElement!.getBoundingClientRect().bottom;
  ...
  return (
    <div className={`drop-down__component w-full ${className} relative`} ref={dropDownEl}>
      <header
