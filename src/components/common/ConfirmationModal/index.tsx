...
const defaultParagraphText = 'Are you sure you want to delete the [NAME] task and its subtaks? the action cannot be reversed.';
    ...
    <PopupLayout onClose={handleReject}>
      <div className="bg-elements max-w-[34rem] p-8 rounded-lg grid gap-6 max-[550px]:p-6">
        ...
        <div className="grid grid-cols-2 gap-4 max-[550px]:grid-cols-1">
          <Button className="w-full text-md [&]:py-3" buttonType="destructive" onClick={handleAccept}>
            Delete
          </Button>
          <Button className="w-full text-md [&]:py-3" buttonType="secondary" onClick={handleReject}>
            Cancel
          </Button>
        </div>
      </div>
